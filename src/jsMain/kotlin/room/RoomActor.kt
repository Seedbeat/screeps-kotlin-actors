...
import actors.ActorIntentBase
import actors.base.*
import memory.planningCache
import memory.stage
import room.planning.RoomPlanningAnalyzer
import room.planning.RoomStagePlanner
...
class RoomActor(
    ...
    private val intentService = RoomIntentService(api = this)
    ...
    override suspend fun processRequest(msg: RoomRequest<*>): RoomResponse<*> = when (msg) {
        is RoomRequest.TryAcquireResourceById -> RoomResponse.TryAcquireResource(
            result = semaphoreService.tryAcquireResource(msg.ownerId, msg.resourceId)
        )

        is RoomRequest.TryAcquireResourceByType -> RoomResponse.TryAcquireAnyResource(
            result = semaphoreService.tryAcquireAnyResource(msg.ownerId, msg.near, msg.type)
        )

        is RoomRequest.ReleaseResourceById -> RoomResponse.ReleaseResource(
    ...
    override suspend fun executeIntent(intent: RoomIntent, time: Int): IntentResultType = when (intent) {
        is RoomIntent.EnsureControllerSurvival -> intentService.ensureControllerSurvival()
        is RoomIntent.PlanWorkforce -> intentService.planWorkforce()
    }
