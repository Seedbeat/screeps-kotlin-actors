...
import room.RoomRequest.ReleaseResourceById
import room.RoomRequest.TryAcquireResourceByType
...
class CreepAssignmentService<T>(
    api: T
) : ActorApi by api,
    ActorBinding<Creep> by api,
    ILogging by Logging<CreepAssignmentService<T>>(api.id, LogLevel.INFO)
        where T : ActorApi,
    ...
    private suspend fun executeEnergyTransfer(assignment: CreepAssignment.EnergyTransfer) {
        ...
        executeHarvestBasedWork(assignment, structure) {
            ...
            when (assignment.goal) {
                is CreepAssignment.EnergyTransfer.Goal.UntilFull -> transfer(target, RESOURCE_ENERGY)
                is CreepAssignment.EnergyTransfer.Goal.Amount -> transfer(target, RESOURCE_ENERGY, assignment.goal.amount)

                is CreepAssignment.EnergyTransfer.Goal.Percent ->
